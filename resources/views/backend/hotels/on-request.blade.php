    ...
    @endif</li>
        ...
        <div class="content content-rest">
          ...
          <div class="restaurants-tabs">
            <ul>
              <li><a href="{{url('/booking-texts',$hid)}}">Regular Price</a></li>
              <li  class="active"><a href="{{url('/on-request',$hid)}}">On Request</a></li>
          	...
          	<div class="details-form">
				...
				<form action="{{route('on-request-update',$hid)}}" method="post" id="request_form">
					@csrf
					 <div class="field">
          ...
          <div class="details-form" style="max-width: 80%;">
            <h3 class="form-name">Check-in Texts</h3>
              ...
              <div class="field">
                ...
                <textarea id="lbl-034" rows="10" name="info_check_in" >{{old('info_check_in', isset($rp) ? $rp->info_check_in : '') }}</textarea>
              </div>

              <div class="field">
                <label for="lbl-044">No arrival allowed on</label>
                <textarea id="lbl-055" rows="10" name="no_arrival_allowed_on">{{old('no_arrival_allowed_on', isset($rp) ? $rp->no_arrival_allowed_on : '') }}</textarea>
              </div>

              <div class="field">
                <label for="lbl-054">Required at check-in</label>
                <textarea id="lbl-066" rows="10" name="required_at_check_in">{{old('required_at_check_in', isset($rp) ? $rp->required_at_check_in : '') }}</textarea>
              </div>
          ...
          <div class="details-form">
            <h3 class="form-name">Payments Texts</h3>
              ...
              <div class="field">
                ...
                <textarea id="lbl-018" rows="5" name="payment_information">{{old('payment_information', isset($rp) ? $rp->payment_information : '') }}</textarea>
              </div>

              <div class="field">
                <label for="lbl-004">Price Information</label>
                <textarea id="lbl-019" rows="5" name="price_information">{{old('price_information', isset($rp) ? $rp->price_information : '') }}</textarea>
              </div>

              <div class="field">
                <label for="lbl-004">Important Information for your booking</label>
                <textarea id="lbl-020" rows="5" name="information_booking">{{old('information_booking', isset($rp) ? $rp->information_booking : '') }}</textarea>
              </div>

              <div class="field">
                <label for="lbl-004">Inclusive</label>
                <textarea id="lbl-021" rows="5" name="inclusive">{{old('inclusive', isset($rp) ? $rp->inclusive : '') }}</textarea>
              </div>

              <div class="field">
                <label for="lbl-004">Costs to be paid on Site</label>
                <textarea id="lbl-021" rows="5" name="costs_paid_site">{{old('costs_paid_site', isset($rp) ? $rp->costs_paid_site : '') }}</textarea>
              </div>

              <div class="field">
                <label for="lbl-004">Hotel Policy</label>
                <textarea id="lbl-021" rows="5" name="hotel_policy">{{old('hotel_policy', isset($rp) ? $rp->hotel_policy : '') }}</textarea>
              </div>

              <div class="field">
                <label for="lbl-004">Free Container</label>
                <textarea id="lbl-021" rows="5" name="free_container">{{old('free_container', isset($rp) ? $rp->free_container : '') }}</textarea>
              </div>
            </form>
			</div>
		</div>
